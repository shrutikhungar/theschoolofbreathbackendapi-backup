...
exports.uploadUrl = async (req, res) => {
    try {
      const { fileName, contentType, isPoster } = req.query;
      const filePath = isPoster ? `images/${fileName}` : fileName;
  ...
  exports.uploadFilesV2 = async (req, res, next) => {
    try {
        const { audioFileUrl, imageFileUrl, name, categoryId, description } = req.body;

        if (!audioFileUrl || !imageFileUrl) {
            return res.status(400).send({message: 'Both sound and poster URLs are required.'});
        }
